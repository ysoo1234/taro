...
export function ReadingForm() {
  ...
  const allCardsReady = useMemo(
    () =>
      cards.every(
        (card) => card.label.trim().length > 0 || Boolean(card.imageDataUrl),
      ),
    [cards],
  );
  const canSubmit = allCardsReady && concern.trim().length >= 8 && !isPending;
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!allCardsReady) {
      setError("각 자리마다 카드명을 적거나 사진을 올려주세요.");
      return;
    }
    ...
    >
      <div className="studio-header">
        <div>
          ...
          <p className="mt-3 max-w-[780px] text-[1.02rem] leading-7 text-[var(--ink-soft)]">
            과거, 현재, 미래 순서로 촬영하면 카드 이름과 방향을 자동으로 읽습니다.
            사진 없이 카드명만 적어도 바로 해석할 수 있어요.
          </p>
        </div>
        <Link
          href="/"
      ...
      <div className="studio-submit-row mt-6">
        <p className="text-sm font-bold text-[var(--ink-soft)]">
          {!allCardsReady
            ? "각 자리마다 카드명을 적거나 사진을 올리면 생성할 수 있어요."
